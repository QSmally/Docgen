...
function FormatEntry(Ctx, Conf, Module, Method) {
    ...
    const Source = new URL(Module, Conf.Repository).toString() + `#L${Ctx.Line}`;
...
module.exports = (Files, Module, Tree, Conf) => {
    ...
    const Documentation = new URL(Conf.Out, Conf.Repository);
    ...
    FS.writeFileSync(Path.join(Conf.Out, `${ModuleName}.md`), [
        ...
        `* [Start](${new URL("Index.md", Documentation)})\n` + (Files.map(Module => `* [${Module}](${new URL(`${Module}.md`, Documentation).toString()})`).join("\n")),
